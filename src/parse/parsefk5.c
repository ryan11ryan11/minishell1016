...
/*   Updated: 2024/10/17 12:32:46 by jbober           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*ms_fillnodext(t_data *data, t_node *content, int i, int k)
{
	...
	while (i < k)
	{
		if (data->currstr[i][0] == 60)
		{
			if (!data->currstr[i + 1])
				return (NULL);
			content->infd = ms_strdup(ms_cleanqt(data->currstr[i + 1], 0, 42, 0));
			if (!content->infd)
				return (NULL);
			i += 2;
		}
		else if ((data->currstr[i]) && (data->currstr[i][0] == 62))
		{
			if (!data->currstr[i + 1])
				return (NULL);
			content->outfd = ms_strdup(ms_cleanqt(data->currstr[i + 1], 0, 42, 0));
			if (!content->outfd)
				return (NULL);
		...
		{
			content->cmd[c] = ms_strdup(ms_cleanqt(data->currstr[i], 0, 42, 0));
			if (!content->cmd[c])
				return (NULL);
			i++;
			c++;
